...
| Pocket DAW | source `0.6.38`; latest published `0.6.37`; `.pocketdaw` schema `2` | `apps/pocket-daw/docs/CURRENT_RELEASE_STATUS.md`, generated from `apps/pocket-daw/release-status.json` | `cd apps/pocket-daw; npm run verify:versions; npm test; cargo test --manifest-path src-tauri/Cargo.toml` | Native-only installed Windows alpha. Public itch channel is `samfa12/pocket-daw:windows-installer`. Do not create, publish, or describe a browser/HTML5/Web Audio Pocket DAW. Tauri/TypeScript UI is part of the native Windows app; the DAW release truth is exact installed artifact evidence. |
