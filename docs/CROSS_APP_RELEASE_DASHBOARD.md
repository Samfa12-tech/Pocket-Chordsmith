...
| Pocket DAW | public checkpoint `0.6.34`; current source may contain unreleased post-checkpoint docs/tests/source changes; `.pocketdaw` schema `2` | `apps/pocket-daw/docs/CURRENT_RELEASE_STATUS.md`, generated from `apps/pocket-daw/release-status.json` | `cd apps/pocket-daw; npm run verify:versions; npm test; cargo test --manifest-path src-tauri/Cargo.toml` | Installed Windows alpha. Public itch channel is `samfa12/pocket-daw:windows-installer`; do not publish a browser/HTML5 DAW app, and do not ship a later commit as another `0.6.34` binary. |
