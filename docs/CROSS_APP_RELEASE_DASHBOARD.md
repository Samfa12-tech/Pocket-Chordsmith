...
Last refreshed: 2026-07-05 from local repo metadata, component package files, and generated release-status docs.
...
| Pocket DAW | source `0.6.39`; latest published `0.6.38`; last installed-smoke `0.6.39`; `.pocketdaw` schema `2` | `apps/pocket-daw/docs/CURRENT_RELEASE_STATUS.md`, generated from `apps/pocket-daw/release-status.json` | `cd apps/pocket-daw; npm run verify:versions; npm test; cargo test --manifest-path src-tauri/Cargo.toml` | Native-only installed Windows alpha. Public itch channel is `samfa12/pocket-daw:windows-installer`. Do not create, publish, or describe a browser/HTML5/Web Audio Pocket DAW. Tauri/TypeScript UI is part of the native Windows app; the DAW release truth is exact installed artifact evidence. Source `0.6.39` is not public until updater, GitHub release, bootstrapper, and itch publication complete and verify. |
...
- `apps/pocket-daw/docs/CURRENT_RELEASE_STATUS.md` now records source `0.6.39`, latest public `0.6.38`, and exact installed-smoke `0.6.39` passing on 2026-07-04. Public claims remain bounded by the latest published version until publication metadata changes.
