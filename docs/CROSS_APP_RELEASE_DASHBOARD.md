...
Last refreshed: 2026-07-22 from local repo metadata, component package files, generated release-status docs, and the Pocket Audio sound-profile release candidate.
...
| --- | --- | --- | --- | --- |
| Pocket Chordsmith web composer | `apps/chordsmith-web/pocket_chordsmith_v68_core_bridge.html`; schema `17`, legacy schema `16` | `apps/chordsmith-web/POCKET_CHORDSMITH_CODEX_CONTEXT.md`; `PROJECT_MEMORY.md` for latest itch push notes | `cd apps/chordsmith-web; npm run test:e2e` | Portable browser app. Keep mostly single-file and preserve `PCS1:`, JSON import/export, share codes, MIDI, WAV, sections, tuplets, holds, slides, bass, drums, guitar, melody, rich events, sound profiles, and handoffs. |
...
| Pocket DAW | source `0.6.44` (MCP help hotfix); latest published `0.6.43`; last installed-smoke `0.6.43` exact pass; `.pocketdaw` schema `3` | `apps/pocket-daw/docs/CURRENT_RELEASE_STATUS.md`, generated from `apps/pocket-daw/release-status.json` | `cd apps/pocket-daw; npm run verify:versions; npm test; cargo test --manifest-path src-tauri/Cargo.toml` | Native-only installed Windows alpha. Public 0.6.43 includes the Samples/Sounds Library, Quick Sampler, 16-pad Drum Rack and opt-in crash-isolated Windows x64 VST3 hosting. Public itch channel remains the stable bootstrapper at `samfa12/pocket-daw:windows-installer`; normal updater checkpoints publish exact-tested installers to GitHub without repushing itch. Do not create, publish, or describe a browser/HTML5/Web Audio Pocket DAW. |
| Pocket Audio Core | package `0.2.0` | `packages/pocket-audio-core/README.md`; `docs/SOUND_PROFILE_EVOLUTION_ARCHITECTURE.md`; `docs/POCKET_AUDIO_SOUND_PARITY_MATRIX.md` | `cd packages/pocket-audio-core; npm test; npm run build; npm run verify:family-parity` | Shared profile/runtime/render contract for Standard, Lofi, Chip, Western, Metal, and Funk. Do not claim mastered cross-backend parity until target listening evidence supports it. |
| PCS Format | package `0.2.0` | `packages/pcs-format/README.md`; `docs/SOUND_PROFILE_EVOLUTION_ARCHITECTURE.md` | `cd packages/pcs-format; npm test; npm run build` | Owns PCS1 schemas 16/17, expressive events, profile identity, unknown-field preservation, capability negotiation, migration, and explicit legacy-projection loss reports. |
| Godot addon | addon `1.2.0` in `addons/pocket_chordsmith/` | `addons/pocket_chordsmith/README.md`; `addons/pocket_chordsmith/plugin.cfg`; `addons/pocket_chordsmith/docs/HEADLESS_VALIDATION.md` | Manual/local Godot headless gates in `HEADLESS_VALIDATION.md`; addon package via `tools/package_pocket_chordsmith_addon.gd` | Reusable Godot addon with schema-17 profile and expressive-event preservation. Keep addon-only packaging under `addons/pocket_chordsmith/` and exclude generated `.import`, `.uid`, `.godot/`, export builds, and local caches. Godot validation is manual/local evidence until a pinned hosted CI workflow exists. |
