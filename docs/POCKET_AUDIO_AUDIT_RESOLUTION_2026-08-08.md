...
| F-009 | Release-candidate truth now rejects a published version newer than exact installed smoke unless a complete version-matched exception is recorded. The existing 0.6.44 source-only/no-installer exception is explicit and test-covered. | Guard closed; next installer remains exact-artifact work |
...
| F-019 | Dependabot, CODEOWNERS, dependency review, CodeQL JS/TS/Rust, tracked-secret scan, governance validation, and all-lockfile high-severity audit gates are present. | Automation closed; GitHub secret-scanning settings pending admin |
