...
| Live MCP Edit Range | Sets an ordinary edit range, ranges a selected clip, clears the edit range, or splits clips at the active range in the running app through `pocket_daw_live_apply_commands`. | MCP-observed installed-app editing smoke when range setup or split-range editing needs to happen without broad UI automation. | Normal edit-range control only; confirm `timelineSelection.source` before destructive-looking range operations and keep punch-range setup separate. |
