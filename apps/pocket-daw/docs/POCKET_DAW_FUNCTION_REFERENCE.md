...
| Import Session Folder / Files | Reconciles generic stems/MIDI ZIPs, consolidated Ableton Live or DAWproject exports, and validated Mureka six-stem AAF exports into one session. Equivalent PCM is deduplicated, distinct same-role layers are preserved, audible stems default to 0.82 and companion MIDI references default muted. | The same song was downloaded in several interchange formats or its stems/MIDI companions need exact alignment. | Ableton/DAWproject clip placement, fades, loops, mixer settings, plug-ins and automation are not reconstructed. Failed media is recorded as missing; zero-load audio imports leave the current project unchanged. Arbitrary non-Mureka AAF layouts are not claimed. |
...
| Function | What it does | Use when | AI counterpart notes |
| --- | --- | --- | --- |
| Select Clip / Track | Chooses what the inspector and lower dock edit; Ctrl/Cmd/Shift-click adds or removes clips from the current multi-selection. | Before changing clip transforms, routing, automation, MIDI, audio, Chordsmith section data, drag movement, or bulk clipboard edits. | MCP live status reports runtime multi-selection as `selection.clipIds`/`selection.clips`. The primary clip still anchors inspector-only commands. |
| Move Left / Move Right | Moves the selected clip or selected clip group earlier/later by the current snap step. | Arrangement timing changes. | Check snap mode first. Bar and beat moves intentionally differ. |
| Cut / Copy / Paste / Duplicate | Moves or repeats clips or selected clip groups without altering source media. Group clipboard edits preserve relative clip spacing. | Moving material, repeating sections, building form, copying multi-track MIDI imports, or trying alternate placements. | Cut removes timeline clips and keeps a runtime clipboard copy as one undoable edit; Paste depends on clipboard state. |
...
| Mute Clip / Delete Clip | Mute silences the selected clip or clip group nondestructively; Delete removes the selected clip or clip group from the timeline. | Mute for auditioning alternatives; Delete for removal. | Prefer Mute when the user may want to recover an idea. |
...
| Quantize | Moves MIDI note starts to a selected rhythmic grid, and note-length quantize snaps durations without moving starts. | Tightening starts or cleaning ragged note lengths. | Changes MIDI timing/length data in project state but is undoable; UI, file-first MCP and live MCP share the same command path. |
| Swing And Groove | Applies swing percentages or named groove templates. | Adding feel after basic timing is correct. | Record chosen groove/swing in metadata where possible; AI collaborators can apply these through file-first MCP and live MCP. |
| Velocity And Pitch Transforms | Applies bulk velocity shaping or pitch shifts. | Humanization, leveling, octave/semitone shifts. | Check pitch range after transforms, especially drums/bass; UI, file-first MCP and live MCP share the same command path. |
...
| MIDI To Chordsmith Mapping | Faithful preview reports independent roles, exact source/destination bars, timing resolution/error, sequential A-H packing, source/written/filtered/grouped counts, generated parts, chord compatibility, raw-reference action and fidelity. Arrange mode retains creative overlay mapping. | Choosing between auditable transcription and an editable creative sketch without changing ordinary MIDI import. | Faithful mode supports up to 128 sequential bars and exact DAW chord overlays; schema-16 PCS1 progression copies remain simplified. |
...
| Transient Analysis And Warp Markers | Finds likely transients, creates/clears metadata warp markers, lets individual marker target bars be edited or deleted, can quantize marker targets to 1/4, 1/8, 1/16 or 1/32 grids without changing source anchors, and can apply a source-safe global varispeed rate from those anchors. | Timing/warp prep, loop fitting or rough live-instrument timing correction before pitch-preserving stretch exists. | Warp-rate playback changes pitch with speed. Editing, deleting or clearing warp markers removes derived warp-rate metadata and restores untouched derived rate/source-offset values, while preserving manual edits. |
...
| Record | Captures audio to project-relative WAV media and places the take on timeline, honoring explicit punch and take-lane mode when enabled. | After saving the project and arming the intended track. | Confirm saved path, count-in, take placement, media pool item and reopen persistence. |
| Punch And Take Mode | Limits live recording placement to the visible punch range when Punch is enabled, auto-stops audio and Web MIDI capture at punch-out, and chooses whether the punch replaces overlapping material or creates a new active take lane. | Repeating a section over existing audio or MIDI while preserving earlier passes for auditioning or comping. | Wired for installed-app audio capture and source-level Web MIDI input capture; real hardware MIDI smoke is still required before public claims. |
| MIDI Input Recording | Starts transport, records note on/off events from the first available Web MIDI input onto the selected MIDI track, auto-stops at punch-out when Punch is enabled, and commits the result as a normal MIDI take clip. | Using a connected MIDI keyboard/controller when a performed part should become editable, saveable, exportable MIDI take-lane material. | Requires Web MIDI support in the runtime and a real MIDI input device. Automated tests cover fake-device capture; installed hardware smoke still needs a controller. |
| Manual Recording Latency Offset | Stores a per-live-track millisecond placement offset for future recordings. Positive values place new takes earlier; negative values place them later, while raw WAV media remains unchanged. | Loopback or listening smoke shows a consistent recorded-track timing offset. | Visible and opt-in. It is not automatic device-latency compensation, and each placed take stores requested/applied offset metadata. |
| Take Lanes | Groups alternate audio or MIDI clips into take lanes, creates editable MIDI takes on MIDI tracks, shows a compact lane overview with active/muted/archived segment counts, activates one take or every clip in a take lane, archives/restores takes, comps from playhead and comps the selected take over the active edit range. | Managing repeated passes or alternate MIDI clips without deleting source media. | Archive is not delete. Make Takes requires overlapping selected clips of one type on one track. MIDI Take creates editable piano-roll take clips, MIDI Rec captures Web MIDI input where available, and live/MCP can place punched MIDI take events. |
...
| Live MCP Session Import | Calls `pocket_daw_live_import_session` with a local folder or supported file path and returns format, track, deduplication, tempo-source, partial-load and warning details. | Agent-driven multi-format import without native picker automation. | Only one heavy session import runs at a time. Large AAF folders can take several minutes; verify missing-media state, native playback and save/reopen tempo behavior before claiming success. |
| File MCP Recording Latency Offset | Stores a live track's manual recording latency offset through `set_recording_latency_offset`, accepting seconds or milliseconds. | Preparing recording smoke where a known input/interface delay should affect future take placement. | Explicit project metadata only. Positive values place new takes earlier; negative values place them later, and clips record requested/applied offset metadata. |
| File MCP Recording Input Channel | Stores explicit live-track Mono Ch N, Stereo Ch N-N+1 or future split-mono input assignments through the file-first command path. | Preparing recording smoke or multi-input projects without visual mixer driving. | Writes project metadata only. Split-mono and other non-default maps remain blocked by native-alpha preflight until native channel routing lands. |
| File MCP Take Lane Activation | Groups overlapping audio or MIDI clips into take lanes, places punched MIDI recording takes from note events, activates every non-archived clip in the selected lane, archives/restores takes without deleting media, and splits grouped takes into first comp segments or active edit-range comps through the undoable command path, from file-first MCP or `pocket_daw_live_apply_commands`. | File-first or MCP-observed live take-lane creation, MIDI recording-take placement, audition, archive/restore and comp smoke after repeated takes or alternate MIDI clips exist. | First same-track take-lane workflow over ordinary clips; dedicated stacked lane subtracks, collapse/solo and full polished comp editing still need exact installed-app smoke before release claims. |
| MCP Punch Recording Placement | Places an explicit punch-window clip from an existing raw recording media item through the undoable command path, either with command-provided bars or from the active `set_punch_range` selection. Pass `createTakeLane: true` to preserve overlapping material as an inactive lane while the new punch becomes active. | File-first or MCP-observed live punch/take-lane smoke, especially after installed recording creates a raw project-media WAV. | Places existing media; it does not start native capture by itself. Pair with installed recording smoke before public release claims. |
| Live MCP Recording Options And Transport | Sets Punch on/off, chooses Replace or Take Lane recording mode, starts/stops/toggles installed-app audio recording, and starts/stops/toggles Web MIDI input recording with transport start and punch-out through `pocket_daw_live_control`. | MCP-observed installed-app recording smoke after saving the project, selecting/arming the intended track, choosing the hardware input, and setting the intended punch range. | The no-hardware punch/take-lane smoke asserts and sets these controls but does not perform real microphone/interface capture or real connected-controller MIDI capture. |
| MCP Punch Range Setup | Marks `timeline.selection.source = "punch"` through file-first MCP or live MCP `set_punch_range`. | Preparing punch-placement smoke or installed punch recording tests without timeline UI driving. | Punch range setup is intent metadata only; it does not start recording or place audio by itself. |
| Live MCP Edit Range | Sets an ordinary edit range, ranges a selected clip, clears the edit range, splits clips, crops selected clips, deletes selected clip ranges, or ripple-deletes selected/all-track ranges in the running app through `pocket_daw_live_apply_commands`. | MCP-observed installed-app editing smoke when range setup or source-safe destructive-looking range editing needs to happen without broad UI automation. | Normal edit-range control only; confirm `timelineSelection.source` before destructive-looking range operations and keep punch-range setup separate. |
| Live MCP Audio Clip Actions | Applies audio-clip actions such as Normalize, Analyze Transients, Create/Quantize/Clear Warp Markers, Invert Phase, Reverse, Short Fades, Reset Fades and crossfade helpers in the running app through `pocket_daw_live_apply_commands`. | MCP-observed installed-app audio editing smoke when analysis or source-safe metadata edits need to happen without visual UI driving. | Warp and quantize actions remain metadata-only preparation for future time-stretch; they do not change source samples or claim pitch correction. |
| Recording Input Preflight Diagnostics | Reports whether armed live tracks and saved input channel assignments are valid before native recording starts; file-first MCP summaries expose `recordingFutureCapturePlan` plus `audioTakeSummary.groups[].lanes[]`, and live MCP status exposes `recording.futureCapturePlan` plus `media.audioTakes.groups[].lanes[]` for shared take/session metadata, lane state, segment spans and lane-level clip observation. | Recording will not start, channel assignments need checking, take-lane activation needs observation, or future multitrack hardware smoke is being prepared. | Exposed in diagnostics and MCP summaries. Grouped capture planning and lane observation are source-only and do not claim simultaneous native multitrack recording or the full visual take-lane editor is shipped. |
| Live MCP Recording Latency Offset | Stores a running live track's manual recording latency offset through `pocket_daw_live_apply_commands:set_recording_latency_offset`. | MCP-observed installed-app recording smoke needs a known timing offset prepared without visual UI driving. | Requires the live bridge token. Visible, opt-in placement metadata only; not automatic hardware-latency detection. |
| Live MCP Recording Input Channel | Stores explicit live-track Mono Ch N, Stereo Ch N-N+1 or future split-mono input assignments in the running app through `pocket_daw_live_apply_commands`. | MCP-observed installed-app smoke when recording setup needs to happen without visual UI driving. | Requires the live bridge token; split-mono and other non-default maps remain blocked by native-alpha preflight until native channel routing lands. |
...
| Live MCP Selection Status | Reports the running app's selected track, primary selected clip and full runtime selected clip group in `pocket_daw_live_status.selection`. | MCP-observed arrangement smoke before group Move, Mute, Delete, Copy, Cut, Paste or Duplicate actions. | Read-only; multi-selection is runtime UI state and saved project files do not persist it. |
