...
| File MCP Punch Recording Placement | Places an explicit punch-window clip from an existing raw recording media item through the undoable file-first command path, either with command-provided bars or from the active `set_punch_range` selection. | Punch/take-lane smoke before a full installed punch recording UI exists. | Metadata/placement foundation only; it does not start native recording or prove user-facing punch recording. |
