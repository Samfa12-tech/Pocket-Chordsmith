...
| Recording Input Preflight Diagnostics | Reports whether armed live tracks and saved input channel assignments are valid before native recording starts; file-first MCP summaries expose `recordingFutureCapturePlan` and live MCP status exposes `recording.futureCapturePlan` for shared mono/stereo/split-mono take metadata. | Recording will not start, channel assignments need checking, or future multitrack hardware smoke is being prepared. | Exposed in diagnostics and MCP summaries. Grouped capture planning is source-only and does not claim simultaneous native multitrack recording is shipped. |
