...
| Function | What it does | Use when | AI counterpart notes |
| --- | --- | --- | --- |
| Select Clip / Track | Chooses what the inspector and lower dock edit; Ctrl/Cmd/Shift-click adds or removes clips from the current multi-selection. | Before changing clip transforms, routing, automation, MIDI, audio, Chordsmith section data, drag movement, or bulk clipboard edits. | MCP live tools can select tracks/clips when visual control is unavailable. Multi-selection is runtime UI state; the primary clip still anchors inspector-only commands. |
