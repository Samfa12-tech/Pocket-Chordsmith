...
| File MCP Take Lane Activation | Activates every non-archived clip in the selected grouped audio take lane, archives/restores takes without deleting media, and splits grouped takes into first comp segments or active edit-range comps through the undoable command path, from file-first MCP or `pocket_daw_live_apply_commands`. | File-first or MCP-observed live take-lane audition, archive/restore and comp smoke after repeated takes exist. | Source-only lane audition foundation; it does not create the full visual take-lane editor or installed punch workflow. |
| MCP Punch Recording Placement | Places an explicit punch-window clip from an existing raw recording media item through the undoable command path, either with command-provided bars or from the active `set_punch_range` selection. The file-first MCP path can provide bars directly or use the active range; the live MCP path can place from the active punch range in the running app. | File-first or MCP-observed live punch/take-lane smoke before a full installed punch recording UI exists. | Metadata/placement foundation only; it does not start native recording or prove user-facing punch recording. |
| MCP Punch Range Setup | Marks `timeline.selection.source = "punch"` through file-first MCP or live MCP `set_punch_range`. | Preparing punch-placement smoke or future installed punch recording tests without timeline UI driving. | Punch range setup is intent metadata only; it does not start recording or claim punch-in/out is shipped. |
