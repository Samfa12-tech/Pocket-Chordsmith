...
| Import Session Folder / Files | Reconciles generic stems/MIDI ZIPs, Ableton Live, DAWproject and validated Mureka six-stem AAF exports into one session. Equivalent PCM is deduplicated; six audible stems default to 0.82 and companion MIDI references default muted. | The same song was downloaded in several interchange formats or its stems/MIDI companions need exact alignment. | Companion MIDI supplies the most complete tempo map with fractional BPM up to 999. Keep confirmed `4/4` when bundled meter metadata is inconsistent; arbitrary non-Mureka AAF layouts are not claimed. |
...
| Live MCP Session Import | Calls `pocket_daw_live_import_session` with a local folder or supported file path and returns format, track, deduplication, tempo-source and warning counts. | Agent-driven multi-format import without native picker automation. | Large AAF folders can take several minutes; the tool waits up to five minutes. Verify native playback plus save/reopen tempo behavior before claiming success. |
