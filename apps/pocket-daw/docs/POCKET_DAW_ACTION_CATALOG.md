...
| Add Folder Track | `data-add-track-kind:folder` |  | Adds a timeline organizer track that can be renamed and saved without processing audio. | Use when a growing arrangement needs visible structure before folder-bus routing exists. | Folder tracks are project data, but current behavior is organizational only: no child mute, no audio routing, no sends and no FX. |
| Assign Track Folder | `data-track-folder` |  | Moves a generated, audio, or MIDI timeline lane into or out of an organizational folder. | Use to group lanes visually and reduce timeline clutter without changing audio behavior. | This is organizational metadata only. Do not infer bus routing, child mute, child solo, sends, FX inheritance, or export grouping from the folder assignment yet. |
