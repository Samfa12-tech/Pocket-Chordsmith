...
| Control | Action / Selector | Shortcut | What it does | Use when | AI counterpart notes |
| --- | --- | --- | --- | --- | --- |
| Add Track | `data-action=add-track-open` | T | Opens the add-track panel. | Use to add live audio, MIDI, generated role, bus, or return tracks. | Opening the panel is UI-only; choose a track kind to mutate the project. |
...
| Add Live Vocals | `data-add-track-kind:live-vocals` |  | Adds a mono live audio track intended for vocal recording. | Use before recording vocals in the installed app. | Recording needs a saved project, input selection, arm state, and human audio smoke. |
| Add Live Instrument | `data-add-track-kind:live-instrument` |  | Adds a mono live audio track intended for external instruments. | Use before recording guitar, keys, microphone, or line input. | Avoid monitor feedback; check input/output devices. |
...
| Mute Track | `data-mute-track` | M | Silences a track without deleting clips or routing. | Use to audition arrangements or isolate problems. | Mute affects playback/render, so inspect it before judging missing audio. |
| Solo Track | `data-solo-track` | S | Auditions one or more soloed tracks while suppressing non-soloed tracks. | Use for focused mix checks. | Solo state can make healthy tracks seem silent; check before bug reports. |
