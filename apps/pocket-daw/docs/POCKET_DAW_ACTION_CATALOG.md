...
| Live MCP Edit Range | `pocket_daw_live_apply_commands:set_timeline_selection, set_timeline_selection_to_clip, clear_timeline_selection, split_timeline_selection` |  | Lets the tokened live bridge set an ordinary edit range, range a selected clip, clear the edit range, or split clips at the active range through the same undoable command paths as the timeline UI. | Use during MCP-observed editing smoke when an AI counterpart needs precise range setup or range splitting in the running app without broad UI automation. | This is normal edit-range control, not punch intent. Confirm `timelineSelection.source` before applying destructive-looking range edits. |
