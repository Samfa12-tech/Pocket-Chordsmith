...
| File MCP Take Lane Activation | `activate_audio_take_lane, pocket_daw_live_apply_commands:activate_audio_take_lane` |  | Activates every non-archived clip in the selected grouped audio take lane and mutes sibling lanes in the same take group through the undoable command path, from file-first MCP or the tokened live bridge. | Use for file-first or MCP-observed live take-lane audition smoke after comp splits create multiple clips in each lane. | This is a source-only lane audition foundation. It does not create the full visual take-lane editor or installed punch workflow. |
