...
| Control | Action / Selector | Shortcut | What it does | Use when | AI counterpart notes |
| --- | --- | --- | --- | --- | --- |
| Add Track | `data-action=add-track-open` | T | Opens the Library / Add Track panel grouped by Audio Recording, Instrument / MIDI, Chordsmith Roles, and Routing. | Use to add live audio, MIDI, generated role, bus, or return tracks without hunting through the busier DAW surface. | Opening the panel is UI-only; choose a track kind to mutate the project. Input device and mono/stereo mode are set on record-capable mixer strips after creation. |
