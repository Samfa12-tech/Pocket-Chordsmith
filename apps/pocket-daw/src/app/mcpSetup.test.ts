...
describe("Pocket DAW MCP setup snippets", () => {
  it("builds copy-ready Windows MCP launch snippets", () => {
    ...
    expect(pocketDawMcpCommandLine()).toContain("npm --silent --prefix");
    ...
    expect(claude.mcpServers[POCKET_DAW_MCP_SERVER_NAME]).toMatchObject({
      command: "cmd",
      args: expect.arrayContaining(["/d", "/c", "npm", "--silent", "--prefix", POCKET_DAW_MCP_WORKSPACE, "run", "mcp:pocket-daw"])
    });
