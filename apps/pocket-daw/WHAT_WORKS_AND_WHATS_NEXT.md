# Pocket DAW v0.5.9 - What Works and What's Next
...
Current v0.5.9 local release verification target:
...
Historical installers and debug executables may remain in `releases/`, but they are not current v0.5.9 alpha artifacts unless regenerated by `npm run package:itch`. Native save/open dialogs, native audio import and native MIDI import are implemented through defensive Tauri commands with browser fallbacks, but real packaged Windows behavior still needs manual verification against the exact itch artifact hash before claiming Windows smoke has passed.
