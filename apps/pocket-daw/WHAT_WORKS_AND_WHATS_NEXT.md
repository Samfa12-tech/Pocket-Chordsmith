...
- Quantization for live instruments: early metadata foundation only. MIDI quantization and reusable groove templates now provide the first source-safe timing tools; selected audio clips can write source-preserving transient marker metadata from waveform peaks, convert those transients into source-safe metadata-only warp markers through UI/command/MCP paths, quantize existing warp marker targets to the project 1/16 grid without changing source anchors, draw stored transient markers in media waveform previews, and play/export source-safe varispeed rate/pitch-as-speed clips. Groove matching, pitch-preserving time-stretch and careful playback/export timing correction remain future work.
...
9. Stereo and simultaneous multitrack recording: explicit input assignment, stereo tracks, monitoring controls, low-latency device paths, full take lanes, punch recording, and comping. One-track mono/stereo recording plus input preflight, source-only grouped future-capture planning, grouped-take activation/archive/restore/comp-from-playhead, explicit MCP punch-range setup and helper-level punch-window placement are foundations only; simultaneous capture, user-facing punch regions and full comp editing remain future work.
