# Pocket DAW v0.5.7 - What Works and What's Next
...
Current v0.5.7 local release verification target:
...
releases/itch/pocket-daw-release-manifest-v0.5.7.json
releases/itch/CHECKSUMS_SHA256_v0.5.7.txt
releases/itch/FINAL_RELEASE_VERDICT_v0.5.7.md
```
...
Historical installers and debug executables may remain in `releases/`, but they are not current v0.5.7 release artifacts unless regenerated by `npm run package:itch`. Native save/open dialogs, native audio import and native MIDI import are implemented through defensive Tauri commands with browser fallbacks, but real packaged Windows behavior still needs manual verification against the portable ZIP before claiming public release smoke has passed.
...
## v0.5.7 Itch Release Readiness

- Bumped app/package/native metadata to v0.5.7 while keeping persisted project schema version 2.
- Added portable Windows itch packaging under `releases/itch/pocket-daw-windows-x64-v0.5.7/`.
- Refined timeline defaults for the updater test build: closer default zoom, live zoom slider resizing, and row/ruler layering so track headers scroll under the bar/time ruler instead of covering it.
