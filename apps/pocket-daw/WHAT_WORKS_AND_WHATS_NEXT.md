...
Current v0.5.1 implementation verification on June 11, 2026:

- `npm test`: 25 test files, 83 tests.
- `npm run build`: TypeScript and Vite production build passed.
- `npm run package:preview`: writes the v0.5.1 browser preview zip.
- `npm run verify:release`: runs tests, production build, browser preview packaging and native debug verification when Cargo/Tauri are available.
- `npm run verify:native-release`: additionally runs full Tauri native release bundling.
- Native debug app output: `src-tauri/target/debug/pocket-daw.exe`.
- Native release app output: `src-tauri/target/release/pocket-daw.exe`.
...
`npm run package:preview` writes a browser preview zip to:

```text
releases/pocket-daw-browser-preview-v0.5.1.zip
```

The preview zip is expected to contain `index.html` at the root, built assets, the root docs and the v0.5.1 docs under `docs/`.
...
Historical installers and debug executables may remain in `releases/`, but they are not current v0.5.1 installer artifacts unless rebuilt in this pass. The v0.5.1 native debug executable is built at `src-tauri/target/debug/pocket-daw.exe`. Native save/open dialogs, native audio import and native MIDI import are implemented through defensive Tauri commands with browser fallbacks, but real dialog behavior still needs manual verification in `npm run tauri:dev` or a packaged Windows build before treating it as private-alpha grade.
