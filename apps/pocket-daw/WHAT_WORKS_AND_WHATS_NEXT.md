# Pocket DAW v0.5.5 - What Works and What's Next
...
Current v0.5.5 local release verification target:
...
Historical installers and debug executables may remain in `releases/`, but they are not current v0.5.5 release artifacts unless regenerated by `npm run package:itch`. Native save/open dialogs, native audio import and native MIDI import are implemented through defensive Tauri commands with browser fallbacks, but real packaged Windows behavior still needs manual verification against the portable ZIP before claiming public release smoke has passed.
...
- Persistent cache hydration from disk was the next cache step in v0.5.4; v0.5.5 adds guarded hydration for valid saved WAV cache assets.

## v0.5.5 Itch Release Readiness

- Bumped app/package/native metadata to v0.5.5 while keeping persisted project schema version 2.
- Added portable Windows itch packaging under `releases/itch/pocket-daw-windows-x64-v0.5.5/`.
