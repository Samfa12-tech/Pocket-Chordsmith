...
- Current release truth: `0.6.46` is the published and exact-installed-smoked updater checkpoint. Source `0.6.47` is release-process-only and has no installer/publication. See `docs/CURRENT_RELEASE_STATUS.md`.
...
- Plugin hosting: VST3 beta shipped in 0.6.43 and passed again in the exact public 0.6.46 smoke. Windows x64 instruments and mono/stereo effects run in the crash-isolated sidecar with native-clock notes/automation/transport, state/program/editor support, latency/tails and missing-plugin recovery. VST2, 32-bit bridging, CLAP, AU/AAX/LV2, multi-bus/surround, sidechain inputs, MIDI output, MPE and `.vstpreset` interchange remain deferred. See `docs/VST3_INSTALLATION_GUIDE.md`.
...
11. Extend hosted plug-ins after the VST3 beta: improve compatibility coverage, continuous hosted-parameter automation, routing-aware latency/tails and safer project portability, then evaluate CLAP/LV2/AU/AAX only when their contracts and distribution terms are appropriate.
...
Historical v0.6.6 release-hygiene commands (not the current procedure):
