...
describe("native render cache", () => {
  ...
  it("caches overlapping same-track generated clips as one grouped stem", async () => {
    const project = createDemoProject();
    const clip = project.timeline.clips.find((item) => item.type === "generated-section")!;
    ...
    expect(cache.generatedRegionCount).toBeGreaterThan(1);
    expect(generatedItems).toHaveLength(cache.generatedRegionCount);
    expect(cache.regions).toHaveLength(cache.generatedRegionCount);
    expect(cache.cachedClipIds.has(clip.id)).toBe(true);
    expect(cache.cachedClipIds.has(overlappingClip.id)).toBe(true);
    expect(generatedItems.every((item) => item.metadata?.overlapGroup === true)).toBe(true);
    expect(generatedItems.every((item) => JSON.stringify(item.metadata?.sourceClipIds) === JSON.stringify([clip.id, overlappingClip.id]))).toBe(true);
  });

  it("caches overlapping same-track MIDI clips as one grouped stem", async () => {
    const imported = importMidiFileToProject(createDemoProject(), parseStandardMidiFile(simpleMidiBytes()), "lead.mid");
    ...
    expect(cache.generatedRegionCount).toBe(1);
    expect(generatedItems).toHaveLength(1);
    expect(cache.regions).toHaveLength(1);
    expect(cache.cachedClipIds.has(clip.id)).toBe(true);
    expect(cache.cachedClipIds.has(overlappingClip.id)).toBe(true);
    expect(generatedItems[0].metadata?.overlapGroup).toBe(true);
    expect(generatedItems[0].metadata?.sourceClipIds).toEqual([clip.id, overlappingClip.id]);
  });
  ...
  it("filters stale generated native stems to tracks that are still current", async () => {
    const project = createDemoProject();
    const first = await buildNativeRenderCache(project, nativeRenderCacheSignature(project));
    const firstAssets = nativeGeneratedAssetsByRole(first);

    const edited = cycleBassStep(project, "A", 0);
    const filtered = filterNativeRenderCacheForProject(edited, first, nativeRenderCacheSignature(edited));
    const filteredAssets = nativeGeneratedAssetsByRole(filtered!);

    expect(filtered).not.toBeNull();
    expect(filteredAssets.bass || []).toEqual([]);
    (["drums", "chords", "melody", "guitar"] as const).forEach((role) => {
      expect(filteredAssets[role]).toEqual(firstAssets[role]);
    });
  ...
  it("keeps filtered stale cache playback while attempting fresh live cache refresh", async () => {
    const previousWindow = (globalThis as { window?: unknown }).window;
    (globalThis as { window?: unknown }).window = {
      setInterval: () => 1,
      clearInterval: () => undefined,
    ...
    try {
      ...
      expect(diagnostics.nativeRenderCache.pendingReason).toBeNull();
    } finally {
      (globalThis as { window?: unknown }).window = previousWindow;
      starts.length = 0;
    }
  ...
  it("keeps metadata-only project loads on cached native generated playback", async () => {
    ...
    try {
      ...
      expect(restarted.events.some((event) => !isSilentCachedSidechainTrigger(event))).toBe(true);
      expect(diagnostics.nativeRenderCache.assetRegionCount).toBeGreaterThan(0);
      expect(diagnostics.nativeRenderCache.proceduralFallbackEventCount).toBeGreaterThan(0);
      expect(diagnostics.nativeRenderCache.payloadCoverage).toBe("partial");
    } finally {
      (globalThis as { window?: unknown }).window = previousWindow;
    }
  });
