...
describe("Pocket DAW MCP tools", () => {
  it("lists the structured Pocket DAW tools", () => {
    ...
    expect(JSON.stringify(liveApplySchema?.properties.commands)).toContain("crop_clip_to_timeline_selection");
    ...
    expect(JSON.stringify(liveApplySchema?.properties.commands)).toContain("place_midi_recording_take");
    expect(JSON.stringify(liveApplySchema?.properties.commands)).toContain("notes");
    ...
    expect(JSON.stringify(liveApplySchema?.properties.commands)).toContain("create_take_lane_group");
    ...
    const liveControlSchema = toolList.find((tool) => tool.name === "pocket_daw_live_control")?.inputSchema as { properties: Record<string, unknown> } | undefined;
    expect(JSON.stringify(liveControlSchema)).toContain("set_recording_options");
    expect(JSON.stringify(liveControlSchema)).toContain("record_start");
    expect(JSON.stringify(liveControlSchema)).toContain("record_stop");
    expect(JSON.stringify(liveControlSchema)).toContain("record_toggle");
    ...
    expect(JSON.stringify(applySchema?.properties.commands)).toContain("place_midi_recording_take");
    expect(JSON.stringify(applySchema?.properties.commands)).toContain("create_take_lane_group");
    expect(JSON.stringify(applySchema?.properties.commands)).toContain("clipIds");
