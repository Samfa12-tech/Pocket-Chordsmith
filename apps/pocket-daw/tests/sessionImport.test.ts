...
import { buildSessionImportProject, type SessionImportAudioAsset, type SessionImportBundle, type SessionImportMidiAsset, type SessionImportNoteTrack, type SessionSourceFormat } from "../src/daw/sessionImport";
...
describe("DAW session import", () => {
  ...
  it("imports a standalone DAWproject with audible audio and muted embedded notes, and reports timing mismatch", () => {
    ...
    expect(result.report.warnings).toEqual([expect.stringContaining("companion tempo-map MIDI")]);
  });
  ...
  it("imports an AAF-only audio session without inventing editable MIDI", () => {
    ...
    expect(result.report.warnings).toContain("tempo.mid contains no MIDI notes and was kept out of the editable reference tracks.");
  });
});
