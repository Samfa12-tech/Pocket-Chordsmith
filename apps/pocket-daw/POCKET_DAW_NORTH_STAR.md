...
3. Non-destructive editing foundation: clip gain, fades/crossfades, source offsets, contextual edit menus, stronger split/trim/range selection and source-preserving destructive-looking operations. Source now has a first-class timeline edit range plus Range Clip, Range Loop, Copy Range, Cut Range, Clear Range, generated-section/generated-pattern/audio/MIDI Split Range, Crop Range, Delete Range, same-track Ripple Delete and source-safe Ripple All commands, valid section-backed generated-pattern clips render through the Chordsmith event engine, selected grouped audio takes can be comped over an active edit range without replacing earlier/later active take segments, metadata-only warp markers derived from analyzed audio transients through UI/command/MCP paths, source-safe audio varispeed rate/pitch-as-speed playback through browser/offline/native runtime-audio regions, and a global warp-marker varispeed action that applies audible/exported timing correction without editing source samples; remaining work includes full take-lane organization and pitch-preserving time-stretch/pitch-shift playback tools.
...
- Installed-app-only one-track live recording alpha with source-level mono/stereo mode support and source-only grouped-take activation/archive/restore/comp-from-playhead/range-comp foundations.
...
- Full punch-in/out, take-lane organization and comp editing. Source now has grouped-take activation/archive/restore, first comp-from-playhead split helpers and range-based comp selection over the active edit range, but no installed punch workflow or full take-lane release claim.
