...
installed-smoke version 0.6.46**. Current source is the 0.6.49 native-audio
and validation release candidate; 0.6.48 was prepared but failed exact-installed
Godot pack validation and was not published.
...
Supported inputs are generic audio-stem/MIDI ZIPs, Ableton Live `.als` sessions or ZIPs, `.dawproject` files, and the validated Mureka six-stem AAF layout. Generic arbitrary AAF projects are not claimed. Ableton and DAWproject audio is treated as consolidated zero-origin stems: clip offsets, fades, loops, mixer settings, plug-ins and automation are not reconstructed. Ableton sessions are most portable when their audio has been collected with the session; external sample paths that are absent from the selected session folder cannot be read.

The reconciler fingerprints decoded PCM so equivalent stems from different formats become one audible track while distinct same-role layers such as rhythm and lead guitar remain separate. It prefers dedicated stems over DAW-session copies and companion MIDI over conflicting embedded representations. Audio tracks start audible at `0.82`; editable MIDI reference tracks start muted so the same performance is not doubled. Redundant unchanged tempo events are collapsed in project automation while the original source-event count remains in provenance. Failed stem hydration is marked missing in the project and recorded in import provenance; an import that cannot load any selected stem leaves the current project unchanged.
...
- v0.6.48 was an unpublished candidate; exact-installed Godot pack validation
  found a section-loop duration mismatch. Its immutable receipt remains retained.
- v0.6.49 is the source candidate for consumed-output transport timing,
  ...
  complete waveform peaks, stronger validation diagnostics and exact tail-free
  section-loop exports. Public claims
...
- Published `0.6.40` includes the first user-facing punch/take-lane workflow for one armed live audio track, Web MIDI input capture that starts transport with automatic punch-out onto selected MIDI tracks, live-bridge recording option/start/stop/toggle controls for installed-app smoke, audio/MIDI take-lane timeline activation/archive/split behavior, export filtering, and the July 2026 timeline/playback hardening fixes. The public checkpoint has exact-artifact smoke attestation, strict audible microphone plus connected loopMIDI evidence, GitHub updater metadata, and itch release metadata.
