...
- The loose root addon zip `pocket_chordsmith_godot_addon_1.1.8.zip` was moved out of the repository front door to ignored local storage at `local-artifacts/archive/packages/godot-addon/root-cleanup-2026-07-05/pocket_chordsmith_godot_addon_1.1.8_root-2026-06-29.zip` because a different `1.1.8` package already existed under the local Godot-addon archive. Do not restore release zips to the repo root.

### Current 2026-07-01 Notes

- Lofi & Chill CD Baby upload was completed on 2026-07-01 from the Pocket Release Mastering Assistant output. The final local archive is `local-artifacts/archive/music/cd-baby/lofi-and-chill-cd-baby-uploaded-2026-07-01.zip` with SHA-256 `611079EFE35C44ECFC03F06F57BC596F5AB9B920BE951DAFE50FB4D5AFCB84C5`. The archive includes the uploaded 16-bit/44.1 kHz stereo WAV set, manifests, reports, mix patches, master settings, source-project copies, extended source-project copies, and short-track extension notes.
- The Lofi & Chill closeout reclaimed local disk space by deleting rebuildable/generated working folders from `local-artifacts/archive/music/lofi-and-chill/supporting-files/`: stems, premaster WAVs, duplicate WAV24 masters, Spotify-native WAV24 copies, extended remaster scratch output, and the unpacked CD Baby upload WAV folder. Keep the archive plus small reports/settings/source JSON as the local release evidence.
...
- The compilation closeout archive lives at `local-artifacts/archive/music/samfa12-compilation-album-1/`. Keep the three ZIPs plus `.sha256.txt` files as local evidence: CD Baby WAV16 upload package, Spotify WAV24 master package, and Pocket DAW/source archive. Also keep `local-artifacts/archive/music/samfa12-compilation-album-1/pocketdaw-projects/` as loose direct-open `.pocketdaw` copies and `local-artifacts/archive/music/samfa12-compilation-album-1/cdbaby-wav16/` as loose CD Baby upload WAVs. The rebuildable staging folder was deleted after archive verification to reclaim about 3.8 GB.
...
- Keep local references, release evidence, package archives, and scratch output under the ignored `local-artifacts/` taxonomy; do not recreate root-level `web-app/`, `pocket_dj/`, `godot-addon/`, `archive/`, `release/`, `releases/`, or `marketing-assets/` folders.
- The old standalone Pocket DAW checkout is archived at `local-artifacts/archive/references/legacy-checkouts/pocket-daw-standalone-2026-06-13/`; do not use a root-level `pocket-daw/` folder for active work.
