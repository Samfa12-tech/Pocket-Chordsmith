...
- `Push to Godot` in the browser app: sends a `PCS1:` code directly to the local addon receiver when Godot is open on the same device, with clipboard/paste fallback. From a phone, use Chordsmith's Mobile transfer or `https://samfa12.com/apps/pocket-audio-handoff/`. The transfer page can issue a short `SAM-...` code for desktop pickup when the relay is available; otherwise paste or upload the `PCS1:` code here.
